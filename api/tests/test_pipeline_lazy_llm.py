from app.text_pipeline.pipeline import TextProcessingPipeline
...
def test_pipeline_does_not_initialize_llm_when_disabled(mocker):
    ...
    pipeline = TextProcessingPipeline(dictionary=mocker.Mock())

    assert pipeline._phase2_llm_corrections('texto qualquer') is None
...
def test_pipeline_initializes_llm_lazily_when_enabled(mocker):
    ...
    assert pipeline._phase2_llm_corrections('texto qualquer') == {'caza': ['casa']}
