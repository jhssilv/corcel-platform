...
import { bulkAssignTexts, bulkUnassignTexts, getFilteredTextsData, getUsernames } from './api/APIFunctions';
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const userOptions = (usernamesResponse.usernames || []).map((username) => ({ value: username, label: username }));
