...
const GeneratedCandidates = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const isInsideFloating = floatingListRef.current?.contains(eventTarget) ?? false;
      const isInsidePanel = sidePanelRef.current?.contains(eventTarget) ?? false;
  ...
  const handleCandidateSelection = async (candidate: string) => {
    ...
    if (!candidate) {
      ...
      await postNormalization(essayId, selectedStartIndex, effectiveEndIndex, candidate, suggestForAll);
    }
