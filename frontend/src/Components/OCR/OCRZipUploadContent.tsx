import { type ChangeEvent, type DragEvent, type MouseEvent as ReactMouseEvent } from 'react';
                ...
                {!uploadFile ? (
                    <>
                        ...
                        <Stack direction="vertical" alignX="center" gap={12}>
                            ...
                            <p className={styles.uploadHintText}>Apenas arquivos .zip contendo imagens (PNG, JPG, TIF)</p>
                        </Stack>
                    </>
                ) : (
                    <Stack direction="vertical" alignX="center" gap={16} className={styles.fileInfoContainer}>
                        ...
                        <div className={styles.fileStatus}>
                            {isValidating && <span className={styles.validating}>Validando...</span>}
                            {!isValidating && isValidZip && <span className={styles.validMark}>Valido</span>}
                            {!isValidating && !isValidZip && hasError && <span className={styles.invalidMark}>Invalido</span>}
                        </div>
                        <Button
                            type="button"
                            tier="secondary"
