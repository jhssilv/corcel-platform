...
function EssayDisplay({ essay, refreshEssay }: EssayDisplayProps) {
	...
	const { handleFinishedToggled, handleResetCorrections } =
		UseCorrectionActions(essay, refreshEssay);

	if (!essay) {
		return <h3>Nenhum texto selecionado.</h3>;
	}

	const textNotFinishedAdvice =
		"Este texto ainda está sendo processado." +
	...
	return (
		<div>
			{(essay.processingStatus === "PROCESSING" ||
				essay.processingStatus === "PENDING") && (
				<Banner
					variant="warning"
			...
			<Stack
				...
				alignY="center"
				gap={12}
				className={legendStyles.essayLegend}
			>
				<Stack alignX="center" gap={20} wrap>
					<Stack alignY="center">
						<span
							className={`${legendStyles.essayLegendSwatch} ${legendStyles.essayLegendSwatchDanger}`}
						></span>
						<span>Não Normalizado</span>
					</Stack>
					<Stack alignY="center">
						<span
							className={`${legendStyles.essayLegendSwatch} ${legendStyles.essayLegendSwatchInfo}`}
						></span>
						<span>Substituído</span>
					</Stack>
					<Stack alignY="center">
						<span
							className={`${legendStyles.essayLegendSwatch} ${legendStyles.essayLegendSwatchNeutral}`}
						></span>
				...
				<Stack
					alignX="center"
					className={legendStyles.essayLegendDivider}
