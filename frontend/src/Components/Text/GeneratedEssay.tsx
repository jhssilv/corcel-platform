...
import { Badge, SectionHeader } from "../Generic";
...
const GeneratedEssay = ({
	...
	useEffect(() => {
		...
		Object.keys(currentCorrections).forEach((key) => {
			...
			if (
				!current ||
				!previous ||
				previous.last_index !== current.last_index ||
				previous.new_token !== current.new_token
			) {
		...
		Object.keys(previousCorrections).forEach((key) => {
			if (!currentCorrections[key]) {
				const previous = previousCorrections[key] as
					| CorrectionValue
					| undefined;
	...
	return (
		<div className={styles["essay-container"]} data-testid="essay-container">
			<SectionHeader
				heading={
					<span data-testid="document-title">{essay.sourceFileName}</span>
				}
				actions={
					essay.grade !== undefined && essay.grade !== null ? (
						<Badge
							text={`Nota: ${essay.grade}`}
							variant="secondary"
							size="sm"
							iconPosition="none"
						/>
