...
export async function validateImageZipFile(file: File): Promise<ZipValidationResult> {
  ...
  try {
    ...
    if (!allImages) {
      return { isValid: false, error: 'O ZIP deve conter apenas imagens (png, jpg, tif, tiff).' };
    }
