...
export function UseTextSelection(essay: TextDetailResponse | null): UseTextSelectionResult {
  ...
  const highlightRange = useMemo<HighlightRange | null>(() => {
    ...
    if (directCorrection) {
      ...
      const entries = Object.entries(corrections) as Array<[string, NormalizationMap[string]]>;
