...
export async function authenticateUser(
	...
	const response = unwrapData(
		await apiPublic.post<LoginApiResponse>("/login", { username, password }),
	);
...
export async function getCurrentUser(): Promise<CurrentUserApiResponse> {
	const response = unwrapData(
		await apiPrivate.get<CurrentUserApiResponse>("/me"),
	);
...
export async function logoutUser(): Promise<MessageApiResponse> {
	const response = unwrapData(
		await apiPrivate.get<MessageApiResponse>("/logout"),
	);
